  ...
  (with-temp-buffer
    ...
    (let ((source (buffer-string)))
      (cl-letf (((symbol-function 'org-texmacs--worker-request)
        ...
        (should (equal (org-texmacs-document-body (org-texmacs-document (current-buffer)))
                     ...
                     (expected
                      (list 'document
                            (list 'concat
                                  ...
                                  " ")))
                     (document (org-texmacs-document (current-buffer))))
...
(ert-deftest org-texmacs-document-uri-rich-description ()
  (with-temp-buffer
    ...
    (should
     (equal (org-texmacs-document-body (org-texmacs-document (current-buffer)))
          ...
          (if (equal suffix "")
              (should (equal (org-texmacs-document-body (org-texmacs-document (current-buffer)))
                             '(document (concat (hlink "(math \"hidden\")"
                                                      "ftps://example.org")))))
            (should-error (org-texmacs-document (current-buffer)) :type 'org-texmacs-document-error)))
...
(ert-deftest org-texmacs-document-uri-native-islands ()
  (org-texmacs-test--with-worker
    (with-temp-buffer
      ...
      (let* ((document (org-texmacs-document (current-buffer)))
             (session (org-texmacs-session-open)))
        (unwind-protect
            (progn
  ...
  (with-temp-buffer
    ...
    (let (requests)
      (cl-letf (((symbol-function 'org-texmacs--worker-request)
        ...
        (let ((result (org-texmacs-document (current-buffer))))
...
(ert-deftest org-texmacs-document-explicit-source-and-wrapper ()
  (with-temp-buffer
    ...
    (let* ((source (current-buffer))
           (expected (org-texmacs-document source)))
      ...
      (with-temp-buffer
        ...
        (let ((caller (current-buffer)) (position (point)))
          (should (equal expected (org-texmacs-document source)))
  ...
  (with-temp-buffer
    ...
    (dolist (source (list nil t 1 (buffer-name)))
      (should-error (org-texmacs-document source)
                    :type 'org-texmacs-document-error))
    (should-error (org-texmacs-document) :type 'wrong-number-of-arguments)
    (let ((dead (with-temp-buffer (current-buffer))))
      (should-error (org-texmacs-document dead)
                    :type 'org-texmacs-document-error))))
        ...
        (let ((caller (current-buffer)) (calls 0))
          (cl-letf (((symbol-function 'org-texmacs--worker-request)
                     (lambda (_source)
                       ...
                       (set-buffer caller)
                       '(math "x"))))
            (let ((result (org-texmacs-document source)))
        ...
        (cl-letf (((symbol-function 'org-texmacs--worker-request)
                   (lambda (_source)
                     ...
                     (pcase change
                       ...
                       ('mode (fundamental-mode)))
                     '(math "x"))))
          (should-error (org-texmacs-document (current-buffer)) :type 'org-texmacs-error)
          (should (= calls 1)))))))
...
(ert-deftest org-texmacs-document-source-root-mismatch ()
  (with-temp-buffer
    ...
    (cl-letf (((symbol-function 'org-texmacs--worker-request)
      ...
      (should-error (org-texmacs-document (current-buffer)) :type 'org-texmacs-parse-error))))
  ...
  (org-texmacs-test--with-worker
    (with-temp-buffer
      ...
      (insert "* Title\nText (math (frac \"α\" \"2\")) end.\n\n"
              "#+begin_texmacs\n(document (math \"中\"))\n#+end_texmacs\n")
      (let ((source (buffer-string))
            (result (org-texmacs-document (current-buffer))))
        (should (equal (org-texmacs-document-body result)
                       '(document (section "Title")
      ...
      (let ((calls 0))
        (cl-letf (((symbol-function 'org-texmacs--worker-request)
                   (lambda (_source)
                     ...
                     '(math "x"))))
          (let ((failure (should-error (org-texmacs-document (current-buffer))
                                       :type 'org-texmacs-document-error)))
    ...
    (let ((session (org-texmacs-session-open))
      ...
      (unwind-protect
          (with-temp-buffer
            (org-mode)
            (insert "Original\n")
            (org-texmacs-session-set-document session (org-texmacs-document (current-buffer)))
            (let ((old (org-texmacs--session-read session)))
      ...
      (cl-letf (((symbol-function 'org-texmacs--worker-request)
        ...
        (should (equal (org-texmacs-document-body (org-texmacs-document (current-buffer)))
                       '(document (section (concat (strong "TODO") " " "[#A]" " "
                                                    "Title" " " ":tag:"))
                                  (concat (math "x") " "))))
        (should (equal (cadr (org-texmacs-document-body (org-texmacs-document (current-buffer))))
                       '(section "changed")))))))
